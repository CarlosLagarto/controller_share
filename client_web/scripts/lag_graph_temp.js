...
let Chart = function(options){
    let _this = this;
    this._nowTS = Math.floor(moment().unix()/60);
    ...
    this.render = function(tsNow, value) {
        ...
        duration = tsNow - _lastTS; //tempo decorrido desde a ultima atualização
        _this._lastTS = tsNow;
...
let Wind = function(options){
    ...
    this.dataLimits = [{r:Math.log10(160),class:"tempestade"},
                       {r:Math.log10(80) ,class:"forte"},
                       {r:Math.log10(40) ,class:"moderado"},
...
let GaugeChart = function(props) {
    ...
    this.render = function() {
        ...
        x = self.outerRadius;
        y = self.outerRadius + 10;
        self.addText(x, y, "Variação Pressão", "middle", "diff-press-first-line");
        y = y + 12;
        this.hPaText = this.addText(x, y, "0.0 hPa/hora", "middle", "diff-press-second-line");
        this.hPaText.attr("id","hPaVelocityValue");
        this.text
    ...
    this.update = function(p) {
        ...
        this.needle.update(this.diffScale(p));
        if (p === undefined){
            this.hPaText.text("-.- hPa/hora");
        }else{
            this.hPaText.text(`${p.toFixed(1)} hPa/hora`);
        }
    }
};
...
let optionsPressure = {
    ...
    maxY : 1085,  //1084.5 - valor record registado em  Tosontsengel, Mongolia on 19 December 2001
...
let optionsWind = {
    ...
    chartId : "#wind-chart",
    maxWind : 180,  //177 foi o maior valor registado em portugal de acordo com o site ipma
    minWind : 0,
    displayUnit : " km/h",
    roundDecimals : 0,
};
